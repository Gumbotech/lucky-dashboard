...
const AstrologicalCalendar = ({ predictions, onDateSelect, dateCellRender, isDateWithinRange }) => (
  <Card
    title={
  ...
  >
    <Calendar
      ...
      disabledDate={(date) => !isDateWithinRange(DateTime.fromJSDate(date.toDate()))}
...
export default function UserDetailsPage({ user, onBack }) {
  ...
  const handleDateSelect = (date) => {
    ...
    if (luxonDate >= today && luxonDate <= thirtyDaysFromNow) {
  ...
  const isDateWithinRange = (date, daysFromToday = 30) => {
    const today = DateTime.now().startOf('day');
    const rangeEnd = today.plus({ days: daysFromToday });
  ...
  const dateCellRender = (date) => {
    ...
    const isValidDate = luxonDate >= today && luxonDate <= thirtyDaysFromNow;

    return (
      <div style={{ backgroundColor: isValidDate ? 'lightgreen' : 'transparent', padding: '4px', borderRadius: '4px' }}>
        {isValidDate && <div style={{ height: '100%', opacity: 0.5 }} />}
        {prediction && (
          <Badge
            color={
