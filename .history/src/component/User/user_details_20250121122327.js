...
const AccountInformation = ({ user }) => (
  <Card style={{ marginBottom: 24 }}>
    <Descriptions title="Account Information" bordered column={1}>
      ...
      <Descriptions.Item label="Date of birth">{user.number}</Descriptions.Item>
      <Descriptions.Item label="Date of birth , Place of birth">{user.description}</Descriptions.Item>
...
export default function UserDetailsPage({ user, onBack }) {
  const [selectedDate, setSelectedDate] = useState(DateTime.now()); // Ensure Luxon DateTime for selected date
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [predictionType, setPredictionType] = useState('normal');
  const [description, setDescription] = useState('');
  const [predictions, setPredictions] = useState(initialPredictions); // Manage predictions dynamically
