...
import { Badge, Button, Calendar, Card, Descriptions, Input, Modal, Radio, Tag, Typography } from 'antd';
...
export default function UserDetailsPage({ user }) {
  ...
  const dateCellRender = (date) => {
    ...
    // Start by rendering the light green background for eligible dates
    const cellContent = [];
    if (isDateWithinRange(luxonDate)) {
      cellContent.push(
        <div
          key="highlight"
          style={{
            backgroundColor: '#ccedcc', // Light green tint for eligible dates
            width: '100%',
            height: '4px',
            position: 'absolute',
            bottom: -2,
    ...
    if (prediction) {
      ...
      cellContent.push(
        ...
        style={{
          ...
          width: '100%', // Take the full width to help with centering
          textAlign: 'center', // Center the text
          borderRadius: '4px',
          padding: '2px 0', // Optional: adjust padding for a cleaner look
        }}>
          {prediction.type}
        </Tag>
      );
