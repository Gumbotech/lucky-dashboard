...
export default function UserDetailsPage({ user, onBack }) {
  ...
  const dateCellRender = (date) => {
    ...
    if (luxonDate >= today && luxonDate <= thirtyDaysFromNow) {
      return (
        <div style={{
          backgroundColor: '#e6f7ff', // Light blue tint for eligible dates
          borderRadius: '50%',
          width: '100%',
          height: '100%',
          display: 'flex',
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px' }}>
      {/* Indicator for eligible dates */}
      {selectedDate >= DateTime.now() && selectedDate <= DateTime.now().plus({ days: 30 }) && (
        <Button type="primary" style={{ marginBottom: '16px' }}>
          Selected Date is within the next 30 days!
        </Button>
      )}
