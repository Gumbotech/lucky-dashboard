...
const AstrologicalCalendar = ({ predictions, onDateSelect, dateCellRender }) => (
  ...
  >
    <Calendar onSelect={onDateSelect} disabledDate={luxonDate >= today && luxonDate <= thirtyDaysFromNow} cellRender={dateCellRender} />
  </Card>
);
...
export default function UserDetailsPage({ user, onBack }) {
  ...
  const dateCellRender = (date) => {
    luxonDate = DateTime.fromJSDate(date.toDate());

    // Get today's date and calculate the range
    today = DateTime.now();
    thirtyDaysFromNow = today.plus({ days: 30 });
