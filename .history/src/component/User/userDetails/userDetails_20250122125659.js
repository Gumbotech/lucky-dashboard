...
export default function UserDetailsPage({ configData, user }) {
  ...
  const fetchUserCalendar = async () => {
    setLoading(true); 
    try {
      const response = await getUserCalendar(user.id, DateTime.now().toFormat('MM-yyyy')); // Fetch data
      ...
      const newPredictions = Object.keys(luckStatus).map(day => {
        const date = `2025-01-${day.padStart(2, '0')}`; // Format date as '2025-01-DD'
        return {
          date,
          type: luckStatus[day],
          description: getDescriptionByType(luckStatus[day]),
        };
      });
