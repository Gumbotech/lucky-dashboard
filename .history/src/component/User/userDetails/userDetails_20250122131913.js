...
export default function UserDetailsPage({ configData, user }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      fetchUserCalendar(DateTime.now().plus({ month: 1 }).toISODate()); 
    }
  }, [user]);
  ...
  const fetchUserCalendar = async (date) => {
    ...
    try {
      ...
      const response = await getUserCalendar(user.id, date.toFormat('MM-yyyy')); // Fetch data
      const luckStatus = response.luckStatus; 

      const month = DateTime.fromFormat(date).toFormat('MM')

      // Convert luckStatus object into an array of predictions
      const newPredictions = Object.keys(luckStatus).map(day => {
        const ndate = `2025-${month}-${day.padStart(2, '0')}`; // Format date as '2025-01-DD'
        return {
          ndate,
          type: luckStatus[day],
          description: '',
