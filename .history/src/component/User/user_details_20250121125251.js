...
const AccountInformation = ({ user }) => (
  <Card style={{ marginBottom: 24 }}>
    <Descriptions title="Account Information" bordered column={1}>
      ...
      <Descriptions.Item label="Time of birth, Place of birth">
        {user.description ?? '--'}
      </Descriptions.Item>
...
const AstrologicalCalendar = ({ predictions, onDateSelect, dateCellRender, onModeChange }) => (
  <Card
    title={
  ...
  >
    <Calendar onSelect={onDateSelect} dateCellRender={dateCellRender} onPanelChange={onModeChange(mode)} />
  </Card>
);
...
export default function UserDetailsPage({ user, onBack }) {
  ...
  const [calendarMode, setCalendarMode] = useState('month'); // Track the calendar mode

  const handleDateSelect = (date) => {
    if (calendarMode !== 'month') return; // Only allow modal in day view
